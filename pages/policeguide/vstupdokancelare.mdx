...
- Dobrý den pane `<hodnost>` (Nejvyšší hodnost v kanceláři), `<Volací Znak>` `<Jméno>` `<Přijmení>` `<Číslo Odznaku>` dostavil jsem se na Vaše předvolání. Sním přístoupit ke stolu?
- Příklad v praxi:
...
- Dobrý den pane `<hodnost>` (Nejvyšší hodnost v kanceláři), `<Volací Znak>` `<Jméno>` `<Přijmení>` `<Číslo Odznaku>` sním přistopit ke stolu a promluvit?
- Příklad v praxi:
